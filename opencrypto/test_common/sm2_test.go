...
import (
	...
	tjsm2 "github.com/rongzer/gmssl-go/opencrypto/gmssl/sm2"

	bccrypto "github.com/rongzer/gmssl-go/crypto"

	gsm2 "github.com/rongzer/gmssl-go/opencrypto/gmssl/sm2"
	tsm2 "github.com/rongzer/gmssl-go/opencrypto/tencentsm/sm2"
	tsm3 "github.com/rongzer/gmssl-go/opencrypto/tencentsm/sm3"
	rzx509 "github.com/rongzer/gmssl-go/x509"
...
func TestSM2Standard(t *testing.T) {
	...
	tjsm2key, _ := rzx509.ParsePKCS8UnecryptedPrivateKey(keyBytes)
	ok := tjsm2key.Verify(digest, sig)
	assert.True(t, ok)

	//rongzer sign, tencentsm verify
...
func TestSM2EncDecStandard(t *testing.T) {
	...
	assert.NoError(t, err)
	tjsm2key, err := rzx509.ParsePKCS8UnecryptedPrivateKey(keyBytes)
	assert.NoError(t, err)
	gsm2key, err := gsm2.UnmarshalPrivateKey(keyBytes)
	assert.NoError(t, err)

	//tencentsm, gmssl, rongzer encryption and decryption
