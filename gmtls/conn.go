...
// 康涅狄格州代表一个安全的连接。它实现了网络。康涅狄格州接口。
// tls安全连接定义, 实现`net.Conn`接口
type Conn struct {
	// constant
	conn        net.Conn
	isClient    bool
	handshakeFn func(context.Context) error // (*Conn).clientHandshake or serverHandshake 客户握手或服务器握手
	...
	// 握手状态是1如果连接是目前转移应用程序数据(即当前没有处理握手)。这个字段只访问同步/原子。
	handshakeStatus uint32
	// constant after handshake; protected by handshakeMutex
	// 握手后不变;保护握手互斥
	handshakeMutex sync.Mutex
	handshakeErr   error   // 错误产生的握手   error resulting from handshake
	vers           uint16  // TLS version
	haveVers       bool    // 版本已协商
	config         *Config // 配置传递给构造函数 configuration passed to constructor

	// 该连接到目前为止执行的握手次数,如果renegotiation被禁止，则该值只能是0或1。
	// handshakes counts the number of handshakes performed on the
	// connection so far. If renegotiation is disabled then this is either
	// zero or one.
	// 握手握手次数的计数进行连接。如果谈判是禁用这要么是0或1。
	handshakes       int
	didResume        bool     // 此连接是否是会话恢复
	cipherSuite      uint16   // 该连接使用的密码套件
	ocspResponse     []byte   // 装订的 OCSP 响应, 由服务端提供给客户端用于检查证书是否已撤销
	scts             [][]byte // 签署证书服务器的时间戳    signed certificate timestamps from server
	peerCertificates []*x509.Certificate

	// verifiedChains是连接握手过程中构建的经过验证的证书链，而非服务端提供的证书链。
	// verifiedChains contains the certificate chains that we built, as
	// opposed to the ones presented by the server.
	// 我们建立验证链包含证书链,而不是服务器提供的。
	verifiedChains [][]*x509.Certificate
	// serverName contains the server name indicated by the client, if any.
	// 服务器名称包含客户机服务器名称显示的,如果任何。
	serverName string
	// 安全重协商?
	// secureRenegotiation is true if the server echoed the secure
	// renegotiation extension. (This is meaningless as a server because
	// renegotiation is not supported in that case.)
	// 安全的重新谈判是真的如果服务器响应安全重新谈判扩展。(这是毫无意义的服务器,因为不支持重新谈判。)
	...
	// 恢复秘密是恢复大师秘密处理新会话的机票信息。如果配置nil。会议门票禁用。
	...
	// 票键是活动会话的设置键连接。第一个是用于加密新票,都是试图解密票。
	...
	// 客户端完成首先是真实的如果客户端发送第一个完成消息在最近的握手。这是记录,因为第一个完成信息传播是tls-unique通道绑定值。
	...
	// channel-binding.
	// 客户端和服务器完成包含完完成由客户机或服务器发送的消息在最近的握手。这是保留支持扩展和重新谈判tls-unique通道绑定。
	...
	// input/output
	in, out   halfConn
	rawInput  bytes.Buffer // 原始输入,从创纪录的头 raw input, starting with a record header
	input     bytes.Reader // 应用程序数据等待阅读,从原始输入。下一个  application data waiting to be read, from rawInput.Next
	hand      bytes.Buffer // 握手数据等待读取  handshake data waiting to be read
	buffering bool         // 记录是否在发送缓冲区缓冲  whether records are buffered in sendBuf
	sendBuf   []byte       // 要发送缓冲区的记录等    a buffer of records waiting to be sent

	// bytesSent counts the bytes of application data sent.
	// packetsSent counts packets.
	// 字节发送应用程序发送的数据的字节数。数据包发送数据包。
	bytesSent   int64
	packetsSent int64
	...
	// 重试计数统计的数量连续non-advancing Conn.read收到的记录记录。记录,无论是推进握手,也没有提供应用程序数据。在保护。互斥锁。
	retryCount int
	...
	// in Conn.Write.
	// 主动调用一个原子int32;低比特是被称为。剩下的部分在康涅狄格州的数量了goroutine。写。
	activeCall int32
...
// 对网络的访问。康涅狄格州的方法。不能嵌入网络。康涅狄格州,因为这会出口结构领域。
