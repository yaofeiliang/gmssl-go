...
const (
	...
	// 弃用:SSLv3密码地打破,不再是由这个包。看到golang.org/issue/32716。
	VersionSSL30 = 0x0300
)
...
const (
	maxPlaintext       = 16384        // 最大明文长度 maximum plaintext payload length
	maxCiphertext      = 16384 + 2048 // 最大密文长度 maximum ciphertext payload length
	maxCiphertextTLS13 = 16384 + 256  // 在TLS 1.3最大密文长度 maximum ciphertext length in TLS 1.3
	recordHeaderLen    = 5            // 记录头的长度 record header length
	maxHandshake       = 65536        // 最大握手我们支持(协议马克斯是16 MB) maximum handshake we support (protocol max is 16 MB)
	maxUselessRecords  = 16           // 最大数量的连续non-advancing记录   maximum number of consecutive non-advancing records
...
// tls记录类型
const (
	recordTypeChangeCipherSpec recordType = 20 // 记录类型改变密码规范
	recordTypeAlert            recordType = 21 // 无法进行有效的翻译
	recordTypeHandshake        recordType = 22 // 记录类型握手
	recordTypeApplicationData  recordType = 23 // 记录类型应用程序数据
)

// TLS handshake message types. TLS握手消息类型。
const (
...
// TLS compression types. TLS压缩类型。
const (
	compressionNone uint8 = 0
)

// TLS extension numbers TLS扩展数字
const (
...
// 曲线ID TLS的类型标识符是一个椭圆曲线。看到
// 在TLS 1.3中,这种类型称为命名组,但此时这个库仅支持基于椭圆曲线的团体。4.2.7看到RFC 8446,部分。
type CurveID uint16
...
// TLS Elliptic Curve Point Formats TLS椭圆曲线点格式
...
// TLS CertificateStatusType (RFC 3546) TLS证书状态类型(RFC 3546)
const (
	statusTypeOCSP uint8 = 1
)

// Certificate types (for certificateRequestMsg) 证书请求证书类型(证书请求)
const (
	certTypeRSASign   = 1
...
// 签名算法(内部信号使用)。从225年开始,以避免重叠和TLS 1.2 codepoints (RFC 5246,附录A.4.1),与这些无关。
const (
...
// 直接签字是一个标准的散列值,表明没有pre-hashing应该执行,并直接输入应该签署。它是与Ed25519签名方案相关联的散列函数。
...
// 支持签名算法包含签名和哈希算法的代码广告支持TLS 1.2 +客户你好,TLS 1.2 +证书请求。这两个字段合并与TLS 1.3。注意,在TLS 1.2, ECDSA算法不约束p - 256,等等。
...
const (
	...
	// 金丝雀TLS12降级或降级金丝雀TLS11嵌入服务器随机降级保护如果服务器能够谈判更高版本。4.1.3看到RFC 8446,部分。
...
// 测试只有力下调金丝雀在测试服务器端包括下调金丝雀即使是使用更高版本支持。
var testingOnlyForceDowngradeCanary bool

// ConnectionState records basic TLS details about the connection.
// 连接状态 // TLS握手过程中的连接状态管理用
type ConnectionState struct {
	// Version is the TLS version used by the connection (e.g. VersionTLS12).
	// 使用的版本是TLS版本连接(例如版本TLS12)。
	Version uint16
	...
	// 简历是真实的如果这个连接成功恢复从先前的会话,会话票或类似的机制。
	DidResume bool

	// CipherSuite is the cipher suite negotiated for the connection (e.g.
	// TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256, TLS_AES_128_GCM_SHA256).
	// 密码套件是连接的密码套件谈判(如TLS ECDHE ECDSA AES 128 GCM SHA256, TLS AES 128 GCM SHA256)。
	CipherSuite uint16
...
// 签名方案标识一个签名算法支持TLS。看到RFC 8446, 4.2.3节。
...
// 获得证书回报最好的证书给客户你好信息,违约的第一个元素c。证书。
...
// 如果提供的证书受支持，SupportsCertificate 支持证书 返回 nil
// 发送 ClientHello 的客户端。 否则，它返回一个错误
// 描述不兼容的原因。
//
// 如果此 ClientHelloInfo 已传递给 GetConfigForClient 或 GetCertificate 获得证书
// 回调，此方法将考虑关联的配置。 注意
// 如果 GetConfigForClient 返回不同的 Config，则不能更改
// 由这个方法计算。
//
// 此函数将调用 x509.ParseCertificate，除非设置了 c.Leaf，这可以
// 产生显着的性能成本。
func (chi *ClientHelloInfo) SupportsCertificate(c *Certificate) error {
	...
	// Section 4.4.2.2).
	// 请注意我们目前不支持认证权威证书和签名算法,并且不检查链上的签名的算法(总之是应该,看到RFC 8446, 4.4.2.2节)。
	...
	// 如果客户指定名称他们试图连接,需要有效的证书。
	if chi.ServerName != "" {
		x509Cert, err := c.leaf()
		if err != nil {
			return fmt.Errorf("failed to parse certificate: %w", err)
		}
		// 验证主机名返回nil如果c是一个有效的证书指定的主机。否则它会返回一个错误描述不匹配。
	...
	// 支持RSAFallback返回nil如果证书和连接支持静态RSA密钥交换,否则和不支持的。支持静态RSA的逻辑是完全分离的逻辑支持签署了关键的交流,所以我们只检查它作为后备。
	supportsRSAFallback := func(unsupported error) error {
		...
		// 静态RSA密钥交换的作品与RSA私钥解密一个挑战,不签署,所以检查实现加密私钥。rsa解密。私钥。
		...
		// 最后,需要有一个共同的密码套件使用静态代替ECDHE RSA密钥交换。
	...
	// 如果客户端发送签名算法扩展,确保它支持计划我们可以使用这个证书和TLS版本。
	...
	// TLS 1.3我们做因为支持团体只是ECDHE相关计算,点格式谈判删除,密码套件只有AEAD相关选择,和静态RSA并不存在。
	if vers == VersionTLS13 || vers == VersionGMSSL {
		return nil
	}
	...
	if priv, ok := c.PrivateKey.(crypto.Signer); ok {
		...
		case *sm2.PublicKey:
			...
			//	curve = Curve256Sm2
			//default:
			//	// 支持RSAFallback返回nil如果证书和连接支持静态RSA密钥交换,否则和不支持的。
			//	// 支持静态RSA的逻辑是完全分离的逻辑支持签署了关键的交流,所以我们只检查它作为后备。
	...
	// 确保有一个相互支持的密码套件适用于这个证书。密码套件选择将应用逻辑的逆向选择。参见服务器握手状态。好的密码套件。
...
// describing the reason for the incompatibility.
// 支持返回nil证书服务器是否支持所提供的证书的,证书请求。否则,它将返回一个错误描述不兼容的原因。
func (cri *CertificateRequestInfo) SupportsCertificate(c *Certificate) error {
	...
	for j, cert := range c.Certificate {
		...
		// chain.Leaf was nil.
		// 解析证书如果这不是叶子节点,或者如果链。叶子是零。
		if j != 0 || x509Cert == nil {
			var err error
			// 分析证书 // 解析证书解析单个证书从给定的asn . 1数据。
...
// 构建名称解析和构建c c。证书。证书名称从常见证书名称和主题交替每个叶证书的名称字段。
// 弃用:名字证书只允许将一个证书与给定的名称。离开这一领域nil让图书馆选择第一个兼容的证书链。
...
// 作家互斥锁保护全球所有关键日志作家。很少启用,仅用于调试,所以全球互斥节省空间。
var writerMutex sync.Mutex

// A Certificate is a chain of one or more certificates, leaf first.
// 证书 证书链的一个或多个证书,叶子第一。 // 证书链, 子证书在前
...
// caching strategy.
// lru会话缓存是一个客户端会话缓存实现使用一个lru缓存策略。
...
// is used instead.
// 新LRUClient会话缓存返回客户端会话缓存容量,使用LRU的策略。如果能力 1,默认使用能力。
...
// 添加了提供(会话密钥,cs)对缓存。如果c是nil,会话密钥对应的条目从缓存中删除。
...
// Get返回客户端会话状态值与一个给定的键。它返回(nil,假)如果没有找到价值。
