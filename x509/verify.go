...
const (
	// NotAuthorizedToSign 证书由另一个人签名时的结果
	// 未标记为 CA 证书。
	NotAuthorizedToSign InvalidReason = iota
	// Expired 证书过期时的结果，基于时间
	// 在 VerifyOptions 中给出。
	Expired
	// CANotAuthorizedForThisName 结果是中间或根
	// 证书有一个不包含名称的名称约束
	// 正在检查。
	CANotAuthorizedForThisName
	// TooManyIntermediates results when a path length constraint is
	// violated.
	// 当路径长度约束为 TooManyIntermediates 结果时
	// 违反了。
	TooManyIntermediates
	// IncompatibleUsage 当证书的密钥用法指示时的结果
	// 它只能用于不同的目的。
	IncompatibleUsage
	// NameMismatch 导致父证书的主题名称
	// 与子项中的发行者名称不匹配。
	NameMismatch
)

// CertificateInvalidError 发生奇数错误时的结果。 这个的用户
// 库可能想统一处理所有这些错误。
