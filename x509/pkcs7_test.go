...
func TestPKCS7SM2(t *testing.T) {
	...
	//	t.Fatal(err)
	//}
	pubKey2, err := ReadPublicKeyFromPem(pubkeyPem) // 读取公钥
	if err != nil {
		t.Fatal(err)
	}
	...
	pubKey2 = &priv.Pub
	certpem, err := CreateCertificateToPem(&template, &template, pubKey2, priv)
	if err != nil {
	...
	data, err := PKCS7EncryptSM2(content, []*Certificate{cert}, 0) //sm2.C1C3C2
	if err != nil {
	...
	decryptData, err := pk7Data.DecryptSM2(cert, priv, 0) //sm2.C1C3C2
	if err != nil {
