...
func ReadPrivateKeyFromPem(privateKeyPem []byte, pwd []byte) (*sm2.PrivateKey, error) {
	return sm2.PrivateKeyFromPEM(string(privateKeyPem), string(pwd))
}

func WritePrivateKeyToPem(key *sm2.PrivateKey, pwd []byte) ([]byte, error) {
	pemStrCertPem, err := key.GetUnencryptedPEM()
	return []byte(pemStrCertPem), err
}

func WriteFile(FileName string, data []byte) (bool, error) {
	decode, _ := pem.Decode(data)
	file, err := os.Create(FileName)
	if err != nil {
		return false, err
	}
	defer file.Close()
	err = pem.Encode(file, decode)
	if err != nil {
		return false, err
	}
	return true, nil
...
func ReadPublicKeyFromPem(publicKeyPem []byte) (*sm2.PublicKey, error) {
	//return sm2.PrivateKeyFromPEM(string(privateKeyPem), string(pwd))
	return sm2.PublicKeyFromPEM(string(publicKeyPem))
...
// ReadCertificateRequestFromPem 从 Pem 读取证书请求
...
// CreateCertificateRequestToPem 为 Pem 创建证书请求
...
// ReadCertificateFromPem 从Pem读取证书
...
// CreateCertificateToPem 创建证书Pem
