...
import (
	...
	"github.com/rongzer/gmssl-go/opencrypto/gmssl/sm3"
...
func marshalPublicKey(pub interface{}) (publicKeyBytes []byte, publicKeyAlgorithm pkix.AlgorithmIdentifier, err error) {
	...
	case *sm2.PublicKey:
		...
		//publicKeyBytes = elliptic.Marshal(pub.Curve, pub.X, pub.Y)
		//Curve
		//sm2.namedCurveFromOID()
		// 获取椭圆曲线oid，注意，国标目前没有给出sm2椭圆曲线的oid，这里使用SM2算法的oid代替
		oid := oidPublicKeySM2 //oidNamedCurveP256SM2
		// 设定公钥算法的oid为sm2算法oid
...
// into the program.
// 大小返回长度,以字节为单位,从给定的哈希函数产生的消化。它不需要问题的哈希函数是链接到程序。
...
// 返回一个新散列。哈希计算给定的散列函数。新的恐慌如果哈希函数没有链接到二进制文件。
...
// 散列注册登记一个函数,返回给定的散列函数的一个新实例。这是旨在从包中的init函数调用实现哈希函数。
...
func getSignatureAlgorithmFromAI(ai pkix.AlgorithmIdentifier) SignatureAlgorithm {
	...
	// PSS是极大地负担过重的选项。这段代码迫使他们分为三类通过要求MGF1散列函数总是匹配消息哈希函数
	// (推荐在https:tools.ietf.org/html/rfc3447 # - 8.1节),盐长度与哈希匹配长度,拖车字段默认值。
...
// Ext关键使用代表一组扩展的行动是有效的对于一个给定的键。每一个Ext键使用常量定义一个唯一的行动。
...
type Certificate struct {
	...
	Issuer              pkix.Name // 证书签署者(提供私钥对RawTBSCertificate进行签名)
	Subject             pkix.Name // 证书拥有者(该证书的核心公钥的拥有者)
	NotBefore, NotAfter time.Time // 证书有效期间 Validity bounds.
	KeyUsage            KeyUsage  // 证书公钥的用途

	// Extensions contains raw X.509 extensions. When parsing certificates,
	// this can be used to extract non-critical extensions that are not
	// parsed by this package. When marshaling certificates, the Extensions
	// field is ignored, see ExtraExtensions.
	// 扩展包含原始x扩展。当解析证书时,这可以用来提取非关键扩展不解析这个包。封送处理证书,扩展字段被忽略时,看到额外的扩展。
	...
	// 额外的扩展包含扩展复制,生到任何军事化管理证书。值覆盖任何扩展,否则生产基于其他字段。额外的扩展字段不是当解析证书时,看到扩展。
	...
	// handled.
	// 未处理的临界扩展包含一个扩展id列表,没有(完全)处理解析。
	// 验证将会失败如果这片是空的,除非验证委托给一个操作系统图书馆了解所有关键的扩展。
	// 用户可以访问使用扩展,这些扩展可以删除元素从这片如果他们相信,他们已经处理。
	UnhandledCriticalExtensions []asn1.ObjectIdentifier

	ExtKeyUsage        []ExtKeyUsage           // 公钥扩展用途 Sequence of extended key usages.
	UnknownExtKeyUsage []asn1.ObjectIdentifier // 未知的公钥扩展用途 Encountered extended key usages unknown to this package.

	BasicConstraintsValid bool // 基础约束是否有效，控制 IsCA 与 MaxPathLen 是否有效  if true then the next two fields are valid.
	...
	// not being set.
	// 马克斯路径Len零表明基本约束有效= = true Len = = 0和Max路径应该解释为一个实际的最大路径长度为零。否则,组合是解释为马克斯路径Len没有设置。
	MaxPathLenZero bool
	...
	// Name constraints 名字约束
	PermittedDNSDomainsCritical bool // 如果真的,那么名字约束是至关重要的。 if true then the name constraints are marked critical.
...
// 犯错不支持的算法试图执行一个操作的结果,包括目前未实现的算法。
...
// 约束违反误差结果当一个证书请求的用法是不允许的。例如:检查签名公钥时并不是一个证书签名密钥。
...
// 委托有破碎的根证书(CN = Entrust.net证书颁发机构(2048))这并不是标记为CA证书,因此根据PKIX无效。我们承认这个证书的公钥信息,免除它的基本约束条件。
...
func (c *Certificate) CheckSignatureFrom(parent *Certificate) error {
	...
	// “如果基本约束扩展版本3证书不存在,或扩展存在但c一个布尔不是断言,那么认证证书不得用于验证公钥签名。”(除了委托,见上面的评论委托破碎SPKI)
...
// CheckSignature 验证签名是否是从以下签名的有效签名
// c 的公钥。
...
func checkSignature(algo SignatureAlgorithm, signed, signature []byte, publicKey crypto.PublicKey) (err error) {
	...
	case *sm2.PublicKey:
		...
		//}
		verify, err := pub.Verify(signed, signature)
		fmt.Println(verify)
		if err != nil {
			return err
		}
...
// CheckCRL签名检查crl中的签名是否来自c。
func (c *Certificate) CheckCRLSignature(crl *pkix.CertificateList) error {
	algo := getSignatureAlgorithmFromAI(crl.SignatureAlgorithm) //从人工智能得到签名算法
...
func parsePublicKey(algo PublicKeyAlgorithm, keyData *publicKeyInfo) (interface{}, error) {
	...
	case SM2:
		...
		return sm2.UnmarshalPublicKey(keyData.PublicKey.Bytes)
		//paramsData := keyData.Algorithm.Parameters.FullBytes
		//
...
func parseCertificate(in *certificate) (*Certificate, error) {
	...
	out.SignatureAlgorithm =
		getSignatureAlgorithmFromAI(in.TBSCertificate.SignatureAlgorithm) // 从人工智能得到签名算法 SM3WithSM2

	out.PublicKeyAlgorithm =
		getPublicKeyAlgorithmFromOID(in.TBSCertificate.PublicKey.Algorithm.Algorithm) //从OID获取公钥算法 oidSM2
	var err error
	out.PublicKey, err = parsePublicKey(out.PublicKeyAlgorithm, &in.TBSCertificate.PublicKey) // 解析公钥
	if err != nil {
		return nil, err
	}
...
func signingParamsForPublicKey(pub interface{}, requestedSigAlgo SignatureAlgorithm) (hashFunc Hash, sigAlgo pkix.AlgorithmIdentifier, err error) {
	...
	for _, details := range signatureAlgorithmDetails {
		if details.algo == requestedSigAlgo {
			fmt.Println("details.algo")
			fmt.Println(details.algo)

			fmt.Println("details.pubKeyAlgo")
			fmt.Println(details.pubKeyAlgo)
			fmt.Println("pubType")
			fmt.Println(pubType)
...
// ParseCertificateRequest 解析证书请求解析一个证书请求从给定的asn . 1数据。
...
func parseCertificateRequest(in *certificateRequest) (*CertificateRequest, error) {
	out := &CertificateRequest{
		...
		SignatureAlgorithm: getSignatureAlgorithmFromAI(in.SignatureAlgorithm), //签名算法

		PublicKeyAlgorithm: getPublicKeyAlgorithmFromOID(in.TBSCSR.PublicKey.Algorithm.Algorithm), //公钥算法

		Version:    in.TBSCSR.Version,
		Attributes: parseRawAttributes(in.TBSCSR.RawAttributes),
	}
	fmt.Println("out.PublicKeyAlgorithm")
	fmt.Println(out.PublicKeyAlgorithm)
	fmt.Println(in.TBSCSR.PublicKey)
	var err error
